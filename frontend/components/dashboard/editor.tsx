...
import { TEMPLATE_CATALOG, getTemplateDefinition } from "@/lib/templates";
import type {
  BookingConfig,
  Overview,
  PublicSite,
    ...
    return (
      <DashboardFrame active="editor">
        <div className="dashboard-loading">
          <LoaderCircle className="spin" /> Loading editor…
        </div>
      </DashboardFrame>
    );
  return (
      ...
      <div className="editor-top">
        ...
        <div className="editor-top-actions">
          ...
          <button
            ...
            onClick={() => run("Your Cleanie page is live.", publishPartner)}
            disabled={busy}
          >
            {overview.partner.status === "published" ? "Published" : "Publish"}
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="card-actions">
            ...
            <button
              ...
              onClick={() =>
                run("Service deleted.", () => deleteService(service.id))
              }
            >
              <Trash2 size={15} />
            </button>
...
function GalleryEditor({
  ...
  const select = (file: File, purpose: "before" | "after") =>
    run("Image uploaded.", async () => {
      const image = await uploadMedia(file, purpose);
      ...
      {overview.portfolio.map((item) => (
        <article className="portfolio-row" key={item.id}>
          ...
          <button
            ...
            onClick={() =>
              run("Portfolio item deleted.", () => deletePortfolioItem(item.id))
            }
          >
            <Trash2 size={15} />
          </button>
      ...
      {overview.reviews.map((review) => (
        <article className="manage-card" key={review.id}>
          ...
          <button
            ...
            onClick={() =>
              run("Review deleted.", () => deleteReview(review.id))
            }
          >
            <Trash2 size={15} />
          </button>
...
function BookingEditor({
  ...
  const save = () =>
    run("Booking settings saved.", async () => {
      ...
      await updateBookingConfig(config);
    });
  return (
    <form
      className="editor-form"

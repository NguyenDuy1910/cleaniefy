...
import { PublicApiError, createBooking, getAvailability } from "@/features/booking/client";
...
export function BookingFlow({
  ...
  useEffect(() => {
    if (!interactive || !date) return;
    const controller = new AbortController();
    setLoadingSlots(true); setSlot(""); setError("");
    getAvailability(site.partner.slug, date, service?.id, controller.signal)
      .then(({ slots: nextSlots }) => setSlots(nextSlots))
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess(true);
    } catch (err) { setError(err instanceof PublicApiError ? err.message : "We couldn’t save your booking. Please try again."); } finally { setSubmitting(false); }
  };
  ...
  return <section className="booking-flow" id="book" aria-labelledby="booking-title"><div className="booking-heading"><div><div className="site-kicker">Ready when you are</div><h2 id="booking-title">Book your cleaning</h2></div><span>2 min</span></div><form onSubmit={submit}>
    ...
    <fieldset><legend>2. Choose a day and time</legend><div className="date-list">{dates.map((item) => <button key={formatDate(item)} type="button" className={date === formatDate(item) ? "selected" : ""} onClick={() => setDate(formatDate(item))}><small>{dayNames[item.getDay()]}</small><b>{item.getDate()}</b></button>)}</div><div className="time-list" aria-live="polite">{loadingSlots ? <span className="times-loading"><LoaderCircle className="spin" size={15}/><span>Finding available times</span></span> : slots.length ? slots.map((item) => <button className={slot === item ? "selected" : ""} type="button" key={item} onClick={() => setSlot(item)}>{new Date(item).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</button>) : <span className="times-empty">No times available for this day.</span>}</div></fieldset>

...
export async function updatePartnerTheme(partnerId: string, input: unknown) {
  ...
  const template = data.template ?? current.template;
  const theme = data.theme ?? (data.template ? getTemplateDefinition(template as Parameters<typeof getTemplateDefinition>[0]).theme : current.themeConfig);
  const [config] = await db()
    .update(partnerSiteConfig)
    .set({ template, themeConfig: theme, updatedAt: new Date() })
